...
{
	if (!NT_SUCCESS(BCryptHashData(*std::get<1>(cryp), (PUCHAR) buf, buf_len, 0)))
		throw std::runtime_error("");
}
...
{
	if (!NT_SUCCESS(BCryptFinishHash(*std::get<1>(cryp), (PUCHAR) std::get<3>(cryp).get(), std::get<4>(cryp), 0)))
		throw std::runtime_error("");
